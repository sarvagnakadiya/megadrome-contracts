...
contract DistributeAirdrops is Script {
    ...
    constructor() {
        ...
        MEGA = Mega(abi.decode(jsonConstants.parseRaw(".MEGA"), (address)));
    ...
    function run() public {
        ...
        uint256 megaBal = MEGA.balanceOf(address(airdrop));
        if (sum > megaBal) amounts[0] = amounts[0] - (sum - megaBal); // remove dust from first airdrop if needed
