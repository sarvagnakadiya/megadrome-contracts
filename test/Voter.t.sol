...
contract VoterTest is BaseTest {
    ...
    function testKillGaugeWithRewards() public {
        ...
        uint256 reward = MEGA.balanceOf(address(voter));
        assertGt(reward, 0);
        uint256 claimableBefore = voter.claimable(address(gauge));
        assertApproxEqRel(claimableBefore, reward, 1e6);

        assertEq(MEGA.balanceOf(address(minter)), 0);

        voter.killGauge(address(gauge));
        assertEq(voter.claimable(address(gauge)), 0);
        // Minimal remains from rounding
        assertLt(MEGA.balanceOf(address(voter)), 1e2); // check for dust
        assertEq(MEGA.balanceOf(address(minter)), claimableBefore);

        // zero-out rewards from minter so in a new rebase, new MEGA is minted
        vm.prank(address(minter));
        MEGA.transfer(address(1), 9999999999999999999999999);
        ...
        assertLt(MEGA.balanceOf(address(voter)), 1e2); // check for dust
        assertGt(MEGA.balanceOf(address(minter)), 0);
    }
    ...
    function testCanStillDistributeAllWithKilledGauge() public {
        ...
        assertEq(MEGA.balanceOf(address(gauge)), 0);
        // gauge2 receives distributions
        assertEq(MEGA.balanceOf(address(gauge2)), 4999999999999999999999999);
    }
    ...
    function testNotifyRewardAmount() public {
        ...
        deal(address(MEGA), address(minter), TOKEN_1);
        vm.prank(address(minter));
        MEGA.approve(address(voter), TOKEN_1);

        uint256 minterPre = MEGA.balanceOf(address(minter));
        uint256 voterPre = MEGA.balanceOf(address(voter));

        vm.prank(address(minter));
        vm.expectEmit(true, false, false, true, address(voter));
        emit NotifyReward(address(minter), address(MEGA), TOKEN_1);
        voter.notifyRewardAmount(TOKEN_1);

        uint256 minterPost = MEGA.balanceOf(address(minter));
        uint256 voterPost = MEGA.balanceOf(address(voter));
