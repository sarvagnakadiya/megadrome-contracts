...
contract TestDeploy is BaseTest {
    ...
    function testDeployScript() public {
        ...
        assertTrue(deployCore.voter().isWhitelistedToken(address(deployCore.MEGA())));
        ...
        for (uint256 i = 0; i < len; i++) {
            assertEq(deployCore.MEGA().balanceOf(liquidWallets[i]), liquidAmounts[i]);
        }
        ...
        assertEq(address(airdrop.mega()), address(deployCore.MEGA()));
        assertEq(airdrop.mega().balanceOf(address(airdrop)), deployCore.AIRDROPPER_BALANCE());
