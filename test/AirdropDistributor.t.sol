...
contract AirdropDistributorTest is BaseTest {
    ...
    function _setUp() public override {
        ...
        assertEq(MEGA.balanceOf(address(airdrop)), 0);
    ...
    function testAirdropDistributorDeployment() public {
        ...
        assertEq(address(airdrop.mega()), address(MEGA));
        assertEq(MEGA.balanceOf(address(airdrop)), INITIAL_DISTRIBUTOR_BALANCE);
        assertEq(escrow.balanceOf(address(airdrop)), 0);
    }

    function testAirdropDistributor() public {
        uint256 preMegaBal = MEGA.balanceOf(address(airdrop));
        ...
        assertGe(MEGA.balanceOf(address(airdrop)), sum);
    ...
    function testCannotAirdropIfInsufficientBalance() public {
        ...
        stdstore.target(address(MEGA)).sig("balanceOf(address)").with_key(address(airdrop)).checked_write(sum - 1);

        vm.expectRevert(IAirdropDistributor.InsufficientBalance.selector);
        airdrop.distributeTokens(_wallets, _amounts);
        assertEq(MEGA.balanceOf(address(airdrop)), sum - 1);
    }
