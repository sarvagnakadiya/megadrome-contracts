...
contract LockedManagedRewardTest is BaseTest {
    ...
    function testCannotNotifyRewardIfNotVotingEscrow() public {
        ...
        lockedManagedReward.notifyRewardAmount(address(MEGA), 0);
    }
    ...
    function testNotifyRewardAmount() public {
        deal(address(MEGA), address(escrow), TOKEN_1 * 3);

        vm.prank(address(escrow));
        MEGA.approve(address(lockedManagedReward), TOKEN_1);
        uint256 pre = MEGA.balanceOf(address(escrow));
        vm.prank(address(escrow));
        vm.expectEmit(true, true, true, true, address(lockedManagedReward));
        emit NotifyReward(address(escrow), address(MEGA), 604800, TOKEN_1);
        lockedManagedReward.notifyRewardAmount(address(MEGA), TOKEN_1);
        uint256 post = MEGA.balanceOf(address(escrow));

        assertEq(lockedManagedReward.isReward(address(MEGA)), true);
        assertEq(lockedManagedReward.tokenRewardsPerEpoch(address(MEGA), 604800), TOKEN_1);
        ...
        vm.prank(address(escrow));
        MEGA.approve(address(lockedManagedReward), TOKEN_1 * 2);
        pre = MEGA.balanceOf(address(escrow));
        vm.prank(address(escrow));
        vm.expectEmit(true, true, true, true, address(lockedManagedReward));
        emit NotifyReward(address(escrow), address(MEGA), 604800, TOKEN_1 * 2);
        lockedManagedReward.notifyRewardAmount(address(MEGA), TOKEN_1 * 2);
        post = MEGA.balanceOf(address(escrow));

        assertEq(lockedManagedReward.tokenRewardsPerEpoch(address(MEGA), 604800), TOKEN_1 * 3);
        assertEq(pre - post, TOKEN_1 * 2);
        assertEq(MEGA.balanceOf(address(lockedManagedReward)), TOKEN_1 * 3);
    }
    ...
    function _addLockedReward(uint256 _amount) internal {
        deal(address(MEGA), address(distributor), _amount);
        vm.startPrank(address(distributor));
        MEGA.approve(address(escrow), _amount);
