...
contract VotingEscrowTest is BaseTest {
    ...
    function testDepositForWithManagedNFT() public {
        ...
        assertEq(MEGA.allowance(address(escrow), address(lockedManagedReward)), 0);
        ...
        uint256 pre = MEGA.balanceOf(address(lockedManagedReward));
        ...
        emit NotifyReward(address(escrow), address(MEGA), 604800, reward);
        vm.expectEmit(false, false, false, true, address(escrow));
        emit MetadataUpdate(mTokenId);
        escrow.depositFor(mTokenId, reward);
        uint256 post = MEGA.balanceOf(address(lockedManagedReward));
    ...
    function testCreateLock() public {
        MEGA.approve(address(escrow), 1e25);
    ...
    function testWithdraw() public {
        ...
        uint256 preBalance = MEGA.balanceOf(address(owner));

        skipAndRoll(lockDuration);
        escrow.withdraw(1);

        uint256 postBalance = MEGA.balanceOf(address(owner));
    ...
    function testCannotSplitOverflow() public {
        ...
        uint256 escrowBalance = MEGA.balanceOf(address(escrow));
    ...
    function testCannotDelegateBySigWithInvalidNonce() public {
        ...
        deal(address(MEGA), alice, TOKEN_100K);
