...
contract ManagedNftTest is BaseTest {
    ...
    function testWithdrawManagedWithLockedReward() public {
        ...
        freeManagedReward = FreeManagedReward(escrow.managedToFree(mTokenId));
        assertEq(MEGA.balanceOf(address(freeManagedReward)), 0);
        ...
        assertEq(MEGA.balanceOf(address(escrow)), TOKEN_1 * 2);
        assertEq(MEGA.balanceOf(address(lockedManagedReward)), 0);
    }
    ...
    function testWithdrawManagedWithFreeReward() public {
        ...
        MEGA.approve(address(freeManagedReward), TOKEN_1);
        freeManagedReward.notifyRewardAmount(address(MEGA), TOKEN_1);

        assertEq(escrow.supply(), supply);
        assertEq(MEGA.balanceOf(address(freeManagedReward)), TOKEN_1);
        assertEq(freeManagedReward.tokenRewardsPerEpoch(address(MEGA), 604800), TOKEN_1);
        ...
        uint256 pre = MEGA.balanceOf(address(owner));
        freeManagedReward.getReward(tokenId, rewards);
        uint256 post = MEGA.balanceOf(address(owner));
    ...
    function testWithdrawManagedWithLockedRewardWithDelegatingManagedNFT() public {
        ...
        assertEq(MEGA.balanceOf(address(escrow)), TOKEN_1 * 3);
        assertEq(MEGA.balanceOf(address(lockedManagedReward)), 0);
    }
