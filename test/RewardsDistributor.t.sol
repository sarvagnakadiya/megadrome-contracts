...
contract RewardsDistributorTest is BaseTest {
    ...
    function testInitialize() public {
        ...
        assertEq(distributor.token(), address(MEGA));
    ...
    function testClaimWithExpiredNFT() public {
        // test reward claims to expired NFTs are distributed as unlocked MEGA
        // ts: 608402
        MEGA.approve(address(escrow), TOKEN_1M);
        ...
        assertEq(post - pre, rebase); // expired rebase distributed as unlocked MEGA
    ...
    function testClaimRebaseWithManagedLocks() public {
        ...
        assertLt(MEGA.balanceOf(address(distributor)), 100); // dust
    }

    function testClaimRebaseWithDepositManaged() public {
        minter.updatePeriod(); // does nothing
        vm.startPrank(address(owner2));
        MEGA.approve(address(escrow), TOKEN_10M);
    ...
    function testClaimOnLockedEnd() public {
        ...
        // Rebase should deposit into veNFT one second before expiry
        uint256 balanceBefore = MEGA.balanceOf(address(owner));
        distributor.claim(tokenId);
        assertGt(MEGA.balanceOf(address(owner)), balanceBefore);
    }
}
