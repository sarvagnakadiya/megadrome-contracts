...
contract PoolTest is BaseTest {
    ...
    function deployPoolCoins() public {
        ...
        amounts[1] = 1e25;
        amounts[2] = 1e25;
        mintToken(address(MEGA), owners, amounts);
        mintToken(address(LR), owners, amounts);
        deployFactories();
        factory.setFee(true, 1);
        factory.setFee(false, 1);

        escrow = new VotingEscrow(address(forwarder), address(MEGA), address(factoryRegistry));
    ...
    function increaseLock() public {
        ...
        assertEq(MEGA.balanceOf(address(escrow)), TOKEN_1);
    }
    ...
    function votingEscrowMerge() public {
        ...
        assertEq(MEGA.balanceOf(address(escrow)), 2 * TOKEN_1);
        ...
        assertEq(MEGA.balanceOf(address(escrow)), 3 * TOKEN_1);
    ...
    function deployMinter() public {
        ...
        MEGA.setMinter(address(minter));
        address[] memory tokens = new address[](5);
        tokens[0] = address(USDC);
        tokens[1] = address(FRAX);
        tokens[2] = address(DAI);
        tokens[3] = address(MEGA);
        tokens[4] = address(LR);
    ...
    function deployPoolFactoryGauge() public {
        ...
        MEGA.approve(address(gaugeFactory), 15 * TOKEN_100K);
    ...
    function addGaugeAndVotingRewards() public {
        ...
        MEGA.approve(address(bribeVotingReward), POOL_1);

        bribeVotingReward.notifyRewardAmount(address(MEGA), POOL_1);
    ...
    function createLock2() public {
        ...
        skip(1);
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(MEGA.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        deal(address(MEGA), address(minter), POOL_1);

        vm.startPrank(address(minter));
        MEGA.approve(address(voter), POOL_1);
    ...
    function gaugeClaimRewards() public {
        ...
        gauge.deposit(POOL_1);
        skip(1);
        uint256 before = MEGA.balanceOf(address(owner));
        skip(1);
        uint256 earned = gauge.earned(address(owner));
        gauge.getReward(address(owner));
        skip(1);
        uint256 after_ = MEGA.balanceOf(address(owner));
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        owner3.approve(address(pool), address(gauge), POOL_1);
        owner3.deposit(address(gauge), POOL_1);
        uint256 before = MEGA.balanceOf(address(owner3));
        skip(1);
        owner3.getGaugeReward(address(gauge), address(owner3));
        uint256 after_ = MEGA.balanceOf(address(owner3));
        uint256 received = after_ - before;
        assertGt(received, 0);
