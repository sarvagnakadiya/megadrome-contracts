...
contract MinterTestFlow is ExtendedBaseTest {
    ...
    function testMinterRebaseFlow() public {
        /// epoch 0
        minter.updatePeriod();
        assertEq(MEGA.balanceOf(address(voter)), 0);

        MEGA.approve(address(escrow), TOKEN_100K);
        ...
        minter.updatePeriod();
        assertEq(MEGA.balanceOf(address(voter)), expectedMint);

        address[] memory gauges = new address[](1);
        gauges[0] = address(gauge);

        uint256 epochStart = _getEpochStart(block.timestamp);
        assertEq(MEGA.allowance(address(voter), address(gauge)), 0);
        voter.distribute(gauges);
        assertEq(MEGA.allowance(address(voter), address(gauge)), 0);
        assertApproxEqRel(MEGA.balanceOf(address(gauge)), expectedMint / 2, 1e6);
        assertApproxEqRel(MEGA.balanceOf(address(voter)), expectedMint / 2, 1e6);
        ...
        voter.distribute(gauges);
        assertApproxEqRel(MEGA.balanceOf(address(gauge2)), expectedMint / 2, 1e6);
        assertLt(MEGA.balanceOf(address(voter)), 1e6); // dust
        skipAndRoll(1);
        ...
        /// epoch 2
        skipToNextEpoch(1);
        uint256 balance = MEGA.balanceOf(address(gauge));
        ...
        assertLt(MEGA.balanceOf(address(voter)), 1e6);
        assertApproxEqRel(MEGA.balanceOf(address(gauge)), balance, 1e6);
        assertApproxEqRel(MEGA.balanceOf(address(gauge)), balance, 1e6);
