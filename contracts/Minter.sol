...
import {IMega} from "./interfaces/IMega.sol";
...
contract Minter is IMinter {
    using SafeERC20 for IMega;
    /// @inheritdoc IMinter
    IMega public immutable mega;
    /// @inheritdoc IMinter
    ...
    constructor(
        ...
        mega = IMega(IVotingEscrow(_ve).token());
        voter = IVoter(_voter);
    ...
    function initialize(AirdropParams memory params) external {
        ...
        for (uint256 i = 0; i < _len; i++) {
            mega.mint(params.liquidWallets[i], params.liquidAmounts[i]);
        ...
        uint256 _tokenId;
        mega.mint(address(this), _sum);
        mega.safeApprove(address(ve), _sum);
        for (uint256 i = 0; i < _len; i++) {
    ...
    function calculateGrowth(uint256 _minted) public view returns (uint256 _growth) {
        ...
        uint256 _megaTotal = mega.totalSupply();

        return (((_minted * (_megaTotal - _veTotal)) / _megaTotal) * (_megaTotal - _veTotal)) / _megaTotal / 2;
    }
    ...
    function updatePeriod() external returns (uint256 _period) {
        ...
        if (block.timestamp >= _period + WEEK) {
            ...
            uint256 _emission;
            uint256 _totalSupply = mega.totalSupply();
            ...
            uint256 _balanceOf = mega.balanceOf(address(this));
            if (_balanceOf < _required) {
                mega.mint(address(this), _required - _balanceOf);
            }

            mega.safeTransfer(address(team), _teamEmissions);
            mega.safeTransfer(address(rewardsDistributor), _growth);
            rewardsDistributor.checkpointToken(); // checkpoint token balance that was just minted in rewards distributor

            mega.safeApprove(address(voter), _emission);
            voter.notifyRewardAmount(_emission);

            emit Mint(msg.sender, _emission, mega.totalSupply(), _tail);
        }
    }
}
