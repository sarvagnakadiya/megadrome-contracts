...
contract VeArtProxy is IVeArtProxy {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '"description": "Megadrome Finance is a next-generation AMM that combines the best of Curve, Convex and Uniswap, designed to serve as the Base central liquidity hub. Megadrome NFTs vote on token emissions and receive bribes and fees generated by the protocol.",',
    ...
    /// @dev Calculates the number of digits before the "decimal point" in an NFT's veMEGA balance.
    ///      Input expressed in 1e18 format.
    function numBalanceDigits(uint256 _balanceOf) internal pure returns (int256 digitCount) {
        uint256 convertedveMEGAvalue = _balanceOf / 1e18;
        while (convertedveMEGAvalue != 0) {
            convertedveMEGAvalue /= 10;
            digitCount++;
        }
    }
    ...
    /// @dev Determines the number of lines in the SVG. NFTs with less than 10 veMEGA balance have zero lines.
