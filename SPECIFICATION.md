...
- MEGA: The native token in the Protocol ecosystem. It is emitted by the Minter and is an ERC-20 compliant token.
...
The VotingEscrow contracts allow users to escrow their MEGA tokens in an veMEGA NFT. 
...
of four years. veMEGA NFT vote weights can be used to vote for pools, which in turn
...
There are three states that veMEGA NFTs can be in: `NORMAL`, `LOCKED`, `MANAGED`.
...
amount of veMEGA that was locked to create it. The NFT's voting power will also
...
- Can create a NFT by escrowing MEGA tokens and "locking" them for a time period.
...
- Can withdraw escrowed MEGA tokens once the NFT lock expires (requires normal NFT). 
- Can add to an existing NFT position by escrowing additional MEGA tokens (requires normal or normal permanent or managed NFT).
...
- For (m)veNFT implementations, refer to the official [repository](https://github.com/megadrome-finance/relay/AutoCompounder).
...
are calculated based on the locked and unlocked MEGA one second prior to epoch flip.
veNFTs will earn rebases proportionally based on their contribution to the total locked
MEGA. Rebase claims against expired veNFTs will be distributed as unlocked MEGA to the 
owner of the veNFT. 
...
Locked rewards are MEGA token rewards that have been compounded into the managed NFT (usually rebases but can also include non-MEGA rewards that have converted to MEGA to be compounded into the NFT). This contract functions similar to `PoolFees`, as it separates "reward" MEGA from the "locked" VotingEscrow MEGA. These rewards are not distributed and are returned to `VotingEscrow` when a user withdraws their NFT from a managed NFT. 
